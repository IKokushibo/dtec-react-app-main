    ...
    const fetchData = async () => {
      try {
        ...
        if (user.role !== "STUDENT_OFFICER") {
          await axios.post(
            `/generic-letters/on-click/${form.id}?type=${form.type}`
          );
        }
        const response = await axios.get(`/sfefs/${form.id}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`mt-6 text-center ${user?.role !== "OFFICE_HEAD" ? 'hidden' : ''}`}>
        ...
        {facilitiesForm && getSignature(facilitiesForm, "OFFICE_HEAD") ? (
          <img
            src={getSignature(facilitiesForm, "OFFICE_HEAD")}
      ...
      <div className="mt-6 text-center">
        ...
        <div className="grid grid-cols-3 gap-4 mt-4">
          {/* Chapel In-charge */}
          <div className={user?.role === "CHAPEL_INCHARGE" ? '' : 'opacity-50'}>
            <div className={`mt-4 ${user?.role !== "CHAPEL_INCHARGE" ? "hidden" : ""}`}>
              <button
                onClick={() => setCaptureFingerprint(true)}
                disabled={user?.role !== "CHAPEL_INCHARGE"}
                className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors mx-auto"
              >
                <FingerPrintIcon className="w-6 h-6" />
                <span>Capture Fingerprint</span>
            ...
            {facilitiesForm && getSignature(facilitiesForm, "CHAPEL_INCHARGE") && (
              <img
                src={getSignature(facilitiesForm, "CHAPEL_INCHARGE")}
          ...
          <div className={user?.role === "PHYSICAL_PLANT_INCHARGE" ? '' : 'opacity-50'}>
            <div className={`mt-4 ${user?.role !== "PHYSICAL_PLANT_INCHARGE" ? "hidden" : ""}`}>
              <button
                onClick={() => setCaptureFingerprint(true)}
                disabled={user?.role !== "PHYSICAL_PLANT_INCHARGE"}
                className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors mx-auto"
              >
                <FingerPrintIcon className="w-6 h-6" />
                <span>Capture Fingerprint</span>
            ...
            {facilitiesForm && getSignature(facilitiesForm, "PHYSICAL_PLANT_INCHARGE") && (
              <img
                src={getSignature(facilitiesForm, "PHYSICAL_PLANT_INCHARGE")}
          ...
          <div className={user?.role === "MULTIMEDIA_INCHARGE" ? '' : 'opacity-50'}>
            <div className={`mt-4 ${user?.role !== "MULTIMEDIA_INCHARGE" ? "hidden" : ""}`}>
              <button
                onClick={() => setCaptureFingerprint(true)}
                disabled={user?.role !== "MULTIMEDIA_INCHARGE"}
                className="flex items-center gap-2 px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors mx-auto"
              >
                <FingerPrintIcon className="w-6 h-6" />
                <span>Capture Fingerprint</span>
            ...
            {facilitiesForm && getSignature(facilitiesForm, "MULTIMEDIA_INCHARGE") && (
              <img
                src={getSignature(facilitiesForm, "MULTIMEDIA_INCHARGE")}
